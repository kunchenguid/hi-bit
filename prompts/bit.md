...
- `browser_open_tab` / `browser_navigate` / `browser_list_tabs` / `browser_switch_tab` / `browser_close_tab` / `browser_back` / `browser_reload` - open and steer browser tabs. A creation's own preview and grown-up-approved websites are allowed; other websites are refused.
