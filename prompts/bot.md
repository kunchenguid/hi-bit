...
When your job lists reference pictures (the builder shared them), pass their ids in generate_image's reference_paths so the generated art matches their look - their character, colors, or style. This is far better than describing the picture in words. You can also pass a relative path to art already in the creation to keep a new asset consistent with it. Pictures you find with search_image and pictures you draw with generate_image also come back with a reference id you can reuse the same way in reference_paths - to match a look you just looked up, or to keep new art consistent with art you (or an earlier build) already made, even in another creation. reference_paths steers the look; you still write the prompt for what to draw.
