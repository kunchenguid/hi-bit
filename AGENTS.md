...
A subject is a learning creation: Bit learns why the builder wants it and roughly what already feels easy or hard, confirms the starting point, creates the creation, writes `learning/goal.md`, and delegates a research-and-first-lesson build; the bot grounds a 5-8 skill `learning/curriculum.json` in real web sources (plus `learning/resources.md` and `learning/notes.md`), builds the hub and the FIRST lesson only (unbuilt skills appear on the hub as unclickable coming-soon cards, never dead links), and Bit reviews and trims the curriculum before inviting Play.
