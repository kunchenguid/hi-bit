...
For visual app or creation questions that should use Bit's screen view, verify that Bit calls `view_screen`, the captured image includes chat chrome plus cross-origin live-preview iframe pixels, the shorter screenshot edge is capped at 1024 pixels, and screenshot base64 is absent from on-disk Bit session files, project logbooks, and renderer-bound events.
