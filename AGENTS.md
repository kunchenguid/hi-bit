...
A subject is a learning creation: Bit runs a short goal interview, confirms, creates the creation, writes `learning/goal.md`, and delegates a research-and-first-lesson build; the bot grounds a 5-8 skill `learning/curriculum.json` in real web sources (plus `learning/resources.md`), and Bit reviews and trims it before inviting Play.
Lessons are interactive pages of that creation, built like any build (the `create-lesson` bot skill holds the lesson doctrine; the `teach-subject` Bit skill in `skills-bit/` holds the teaching doctrine and file schemas, read lazily only when a teach ask arrives).
...
Bit also appends learning records (`learning/learning-records/`) on real evidence - demonstrated understanding, disclosed prior knowledge, corrected misconceptions, goal shifts - and the per-turn subjects note carries goal, skill map, and recent records so a fresh session resumes in the zone of proximal development.
Parents get visibility, not an approval gate: subjects appear in the grown-up progress window with goals and precise skill names, and the `learning/` files are plainly inspectable in the creations folder.
