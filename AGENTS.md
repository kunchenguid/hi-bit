...
2. Kill the dev server: stop the `pnpm dev` process you started (the one running `electron-vite dev`). Quitting the Electron window also ends it because the dev server is tied to Electron's lifecycle, but do not rely on a window quit alone - confirm the process is gone. If you launched it in the background, kill that background task by id; otherwise find and kill it (`pkill -f electron-vite` - the broader pattern matters: the actual process is `node .../electron-vite.js dev`, so `pkill -f "electron-vite dev"` matches nothing and silently leaves the app running), then verify nothing still holds the debugging port (`lsof -i :9222` should be empty).
