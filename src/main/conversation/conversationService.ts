 ...
 * and the active Bit/Mayor Pi session file (`conversation.json`).
 */
export class ConversationService {
  ...
  async getMayorSessionFile(profileId: string): Promise<string | undefined> {
    const state = await readJsonFile<ConversationStateRecord>(
      this.paths(profileId).conversationStatePath,
    );
    return state?.activeMayorSessionFile;
  }

  async setMayorSessionFile(profileId: string, sessionFile: string | undefined): Promise<void> {
    await writeJsonFile(this.paths(profileId).conversationStatePath, {
      schemaVersion: 1,
      activeMayorSessionFile: sessionFile,
