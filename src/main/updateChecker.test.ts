...
import { compareSemver, createUpdateChecker, parseLatestRelease } from "./updateChecker";
...
describe("parseLatestRelease", () => {
  it("extracts the version (without v) and release page url", () => {
    expect(
      parseLatestRelease({
        tag_name: "v0.2.0",
        html_url: "https://github.com/kunchenguid/hi-bit/releases/tag/v0.2.0",
      }),
    ).toEqual({
      version: "0.2.0",
      url: "https://github.com/kunchenguid/hi-bit/releases/tag/v0.2.0",
    });
  });
  ...
  it("extracts the version from component-prefixed release tags", () => {
    expect(
      parseLatestRelease({
        tag_name: "hi-bit-v0.2.0",
        html_url: "https://github.com/kunchenguid/hi-bit/releases/tag/hi-bit-v0.2.0",
      }),
    ).toEqual({
      version: "0.2.0",
      url: "https://github.com/kunchenguid/hi-bit/releases/tag/hi-bit-v0.2.0",
    });
  });
...
describe("createUpdateChecker", () => {
  ...
  it("queries the hi-bit releases API with a hi-bit user agent", async () => {
    const fetchImpl = vi.fn(release("v0.2.0"));
    const checker = createUpdateChecker({ currentVersion: "0.1.0", fetchImpl, now: () => 0 });

    await checker.getStatus();
    expect(fetchImpl).toHaveBeenCalledWith(
      "https://api.github.com/repos/kunchenguid/hi-bit/releases/latest",
      expect.objectContaining({
        headers: expect.objectContaining({ "User-Agent": "hi-bit" }),
      }),
    );
  });

  it("reports an update from component-prefixed release tags", async () => {
    const fetchImpl = vi.fn(release("hi-bit-v0.2.0"));
    const checker = createUpdateChecker({ currentVersion: "0.1.7", fetchImpl, now: () => 0 });
  ...
  it("opens the generic releases page when no specific release url is known", async () => {
    ...
    expect(openExternal).toHaveBeenCalledWith(
      "https://github.com/kunchenguid/hi-bit/releases/latest",
    );
  });
});
