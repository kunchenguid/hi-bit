...
async function createCoordinator() {
  ...
  const game = await projects.create({ title: "Factory game" });
  const site = await projects.create({ title: "Space site" });
  return { coordinator, projects, runtime, pipeline, game, site };
}
...
describe("BitCoordinatorService", () => {
  it("turns a lead builder message into a build plan, bot job, workbench run, inspections, and install", async () => {
    ...
    await expect(
      setup.coordinator.send(setup.game.id, "Add star pets", (event) => events.push(event)),
    ).resolves.toEqual({
      ok: true,
      turnId: "turn-1",
      status: "completed",
    ...
    expect(setup.runtime.projects[0]).toMatchObject({
      id: setup.game.id,
      mainWorkbenchDir: join(setup.projects.pathsFor(setup.game.id).workbenchesDir, "bot_job_0001"),
    ...
    const paths = setup.projects.pathsFor(setup.game.id);
    ...
    await expect(setup.coordinator.load(setup.game.id)).resolves.toMatchObject({
  ...
  it("does not run machines or the assembly line when a bot job is cancelled", async () => {
    ...
    await expect(setup.coordinator.send(setup.game.id, "Stop building", () => {})).resolves.toEqual(
      {
        ok: true,
        turnId: "turn-1",
        status: "cancelled",
