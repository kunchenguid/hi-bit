...
The production app now centers on a Codex provider connection with a reconnect overlay, kid profile selection, and a profile-level Pi-backed Bit chat workspace where creations are managed through chat, the composer supports picture and optional local voice input, the Grown-up menu can quietly show update guidance, and creations open in the browser split pane.
