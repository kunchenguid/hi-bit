...
- **For kids:** a warm arcade. Pixel stickers, cheerful build moments, a tutor pal named **Bit** - a curious little desktop-computer robot who leans in and figures things out with you.
...
- **Referenced repo:** `kunchenguid/hi-bit` Electron app with a Codex provider connection and reconnect overlay, kid profiles, a profile-level Pi-backed Bit chat workspace with one-picture messages and optional local voice input, live creation previews, and creations managed through chat.
...
| `ui-kits/desktop/` | Legacy desktop brand prototype - interactive UI kit with JSX components + a click-thru `index.html` |
...
|---|---|---|---|
| HTML | `--subject-html` | `#F26A4B` (coral) | tags, document structure work |
| CSS | `--subject-css` | `#2EC4F1` (sky cyan) | styling, layout work |
...
Semantic colors (`--success`, `--danger`, `--warning`) derive from the subject rainbow - `--success` is lime, `--danger` is coral, `--warning` is amber - so celebrations feel continuous with project work.
...
- **Easing:** `--ease-bounce: cubic-bezier(0.34, 1.56, 0.64, 1)` for arrivals (toasts, build confirmations). `--ease-out: cubic-bezier(0.22, 1, 0.36, 1)` for everything else.
...
- `paper-texture.svg` (subtle grain for hero backgrounds)
...
4. For the current product surfaces, review the renderer screens: Codex connection, Codex reconnect overlay, profile gate, chat workspace, voice-enabled composer, and live preview pane.
